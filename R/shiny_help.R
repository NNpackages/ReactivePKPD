...
#' Return objects from PKPDEnv environment
#'
...
#' @param name The object to fetch from the PKPDEnv
#'
#' @return reactive function with the object stored in PKPDEnv with name: name
#' @export
#' @importFrom shiny reactiveValues isolate observe invalidateLater reactive
checkPKPDEnv <- function(intervalMillis, session = NULL, check, name){
  
  intervalMillis <- coerceToFunc(intervalMillis)
  rv <- shiny::reactiveValues(cookie = shiny::isolate(PKPDEnv(check)))
  
  shiny::observe({
    rv$cookie <- PKPDEnv(check)
...
updateLoginInfo <- function(session.id, user, login = FALSE, logout = FALSE) {
  ...
  session_info <- PKPDEnv("session_info")
  ...
  if (!missing(login) && login) {
    ...
    PKPDEnv("online_user", unique(c(PKPDEnv("online_user"), toupper(user))))
  ...
  if (!missing(logout) && logout) {
    ...
    if (!missing(user))
      PKPDEnv("online_user", setdiff(PKPDEnv("online_user"), toupper(user)))
  ...
  PKPDEnv("session_info", session_info)
  
  PKPDEnv("session_info_last_change", Sys.time())
...
loginInfo <- function(check = FALSE) {
  ...
  if (check) return(PKPDEnv("session_info_last_change"))
