...
#' @importFrom magrittr '%>%'
runPKPDreview <- function(trial = NULL, username = unname(Sys.info()["user"]),
  ...
  appDir <- system.file("review-app", package = "NNPKPD")

  if (appDir == "") {
    stop("Could not find the review app directory. Try re-installing 'NNPKPD'.", call. = FALSE)
  }

  PKPDEnv("admins", admins)
  PKPDEnv("online_user", character(0))
  PKPDEnv("trial", trial)
  PKPDEnv("username", username)
  PKPDEnv("workspace", workspace)
  PKPDEnv("verbose", verbose)

  PKPDEnv("access_data_path", access_data_path)
  PKPDEnv("data_storage", data_storage)
    ...
    if (!is.null(sep2)) {
      if (is.null(which_sep2)) {
        ...
        if (class(which_sep2) == "character") {
          which_sep2 <- match(which_sep2[which_sep2 %in% colnames(data)], colnames(data))
        }
      }
