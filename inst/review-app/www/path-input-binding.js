...
// shiny.js.
var pathInputBinding = new Shiny.InputBinding();


// An input binding must implement these methods
$.extend(pathInputBinding, {

  // This returns a jQuery object with the DOM element
  find: function(scope) {
    return $(scope).find('input[type="path"]');
  },
  ...
  subscribe: function(el, callback) {
    $(el).on('keyup.pathInputBinding input.pathInputBinding', function(event) {
      callback(true);
      // When called with true, it will use the rate policy,
      // which in this case is to debounce at 500ms.
    });
    $(el).on('change.pathInputBinding', function(event) {
      callback(false);
  ...
  unsubscribe: function(el) {
    $(el).off('.pathInputBinding');
  },

  // Receive messages from the server.
  // Messages sent by updatepathInput() are received by this function.
...
Shiny.inputBindings.register(pathInputBinding, 'shiny.pathInput');